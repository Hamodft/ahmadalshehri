...
## Editing the page without touching code

Scroll to the bottom and click **Edit this page** (or add `#edit` to the URL).

- Click any text to change it
- **+ Duplicate** on a position copies it so you can edit the copy into a new job
- **+ Add certificate** and **+ Add skill** work the same way
- **Remove** deletes an item
- **Link** sets the Google Drive URL behind a certificate
- **Save & download index.html** gives you a new file — upload it to GitHub to replace the old one

Nothing saves to the internet by itself. Until you download and upload, changes exist only in your browser, so you can experiment freely. Closing the tab loses them.

**Edit each language separately.** The banner tells you which one you're in. Switch with the header button and edit the other side too, or the two versions drift apart.

## Certificate links

All ten certificates link to your Google Drive files, opening in a new tab. They work because the folder is shared publicly — if you ever set it back to private, the links stop working for visitors.
