...
(function () {
  ...
  function renderBindings() {
    ...
    if (D.profile.photo) {
      el('photoSlot').outerHTML =
        '<div class="profile-photo-stage" role="img" aria-label="' + esc(t(D.profile.name)) +
        '" style="background-image:url(&quot;' + esc(D.profile.photo) + '&quot;)"></div>';
    } else if (mono) {
  ...
  Promise.all(FILES.map(function (f) {
    return fetch('data/' + f + '.json?v=20260819-v12', { cache: 'no-store' }).then(function (r) {
